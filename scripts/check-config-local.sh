...
ERROR=$(
    for database in $(${DOCKER_EXEC_LOCAL} python3 pg2sphinx_trigger.py -s /etc/manticoresearch/manticore.conf -c list | awk '$2 ~ /->/ {print $3}' | sort | uniq); do
        database="${database%%[[:cntrl:]]}"
        ${DOCKER_EXEC_LOCAL} python3 pg2sphinx_trigger.py -s /etc/manticoresearch/manticore.conf -c list -d "${database}." | grep -i ERROR -a4 || continue
    done
)
